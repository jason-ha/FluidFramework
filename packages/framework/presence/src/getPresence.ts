...
function assertCompatibilityInvariants(compatibility: ExtensionCompatibilityDetails): void {
	...
	assert(
		compatibility.generation === presenceCompatibility.generation,
		"ContainerPresenceManager.upgradeVersionOrCapabilities needs to account for new compatibility generation.",
	);
...
class ContainerPresenceManager
	implements
		...
		>
{
	public static readonly instanceRequirements = presenceCompatibility;
...
export function getPresenceFromDataStoreContext(context: IFluidDataStoreContext): Presence {
	...
	return context.getExtension(ContainerPresenceManager.extensionId, ContainerPresenceManager);
}
