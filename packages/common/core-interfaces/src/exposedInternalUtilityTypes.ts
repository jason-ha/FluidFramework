...
export namespace InternalUtilityTypes {
	...
	> = T extends TAncestorTypes
		? /* recursion found => reprocess that recursive type directly to avoid
			 ...
			 Further recursion will not happen during processing. Either the type
			 will require modification and the `TypeUnderRecursion=true` arg will
			 result in `OpaqueJsonDeserialized<T>` wrapper. Otherwise, no change
			 is needed at this point and `T` will result. */
